...
function Zoskron:CreateWindow(title)
    ...
    -- Fullscreen, Minimize, Close (Unicode icons)
    local FullBtn = tnew{
        Class = "TextButton",
        Name = "Full",
    ...
    -- store original size/position for restore after fullscreen/minimize
    ...
    local function setFullscreen(state)
        ...
        if state then
            ...
            tween(Main, {Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0)}, 0.25)
            -- adjust inner frames
            tween(PagesFrame, {Size = UDim2.new(1, -150, 1, -36), Position = UDim2.new(0, 150, 0, 36)}, 0.25)
            tween(Sidebar, {Size = UDim2.new(0, 150, 1, -36), Position = UDim2.new(0, 0, 0, 36)}, 0.25)
        else
            tween(Main, {Size = originalSize, Position = originalPos}, 0.25)
            tween(PagesFrame, {Size = UDim2.new(1, -150, 1, -36), Position = UDim2.new(0, 150, 0, 36)}, 0.25)
            tween(Sidebar, {Size = UDim2.new(0, 150, 1, -36), Position = UDim2.new(0, 0, 0, 36)}, 0.25)
        end
    end
    ...
    function Window:SetBackgroundTransparency(n)
        n = tonumber(n) or 0
        n = math.clamp(n, 0, 1)
        Main.BackgroundTransparency = n
        Config.BackgroundTransparency = n
        saveConfig(Config)
    end
    ...
    function Window:MakeTab(opts)
        ...
        btn.MouseButton1Click:Connect(function()
            for _, p in pairs(Pages:GetChildren()) do
                if p:IsA("Frame") and p.Name:match("_Page$") then
                    p.Visible = false
                end
            end
            page.Visible = true
